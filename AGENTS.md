...
- `src/collector/adapters/` contains provider-specific JSONL normalization. Adapters must never return raw prompts, responses, reasoning, credentials, or tool arguments. The Zcode adapter additionally resolves `cwd`/`repository` for interactive ("rollout") sessions from Zcode's own session database (`~/.zcode/cli/db/db.sqlite`, opened read-only via a cached connection in `zcode.ts`, PK lookup on `session.id == externalId`) because the rollout `model_io` rows carry no working directory; `ZCODE_DB_PATH` is the test seam and `__resetZcodeDbCache` clears the connection between tests. Branch is not enriched from the DB — no git field exists in Zcode's schema — and stays JSONL-derived (`gitBranch` rows, available only for subagent transcripts).
